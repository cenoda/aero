...
> **Status:** Re-opened 2026-06-18 — new findings from final verification review (Round 4).
> Round 1–3 items (below the separator) are complete and archived.
...
- [ ] **R4.1 `TextDocument.Content` swallows exceptions and can return stale data** *(notable)*
...
- [ ] **R4.2 `EditorViewModel.Dispose()` is never called — the leak fix (TOFIX #1) is dead code** *(notable)*
...
- [ ] **R4.3 Static `App.Services` locator contradicts AGENTS.md** *(minor)*
...
- [ ] **R4.4 Unnecessary `using` directives** *(minor)*
...
- [ ] **R4.5 Inconsistent message nullability** *(minor)*
...
- [ ] **R4.6 `CheckDirtyBeforeExitAsync` can hang if `ConfirmDirtyClose` has no subscriber** *(minor / theoretical)*
...
- [ ] **R4.7 Unused public API surface on `TextDocument`** *(minor / optional)*
